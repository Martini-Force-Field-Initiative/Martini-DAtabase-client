...
const martini3ColorScheme: {
  [beadType: string]: { color: string; hydrophobicity: number };
} = require("./schemes/martini3_cyan_orange.json");
const martini2ColorScheme: {
  [beadType: string]: { color: string; hydrophobicity: number };
} = require("./schemes/martini2_cyan_orange.json");
const martini3BeadsAA: {
  [bead: string]: { aminoAcid: string; atomName: string }[];
} = require("./schemes/martini3_beads_aa.json");
...
const MARTINI3_BEADS_REGEX = new RegExp(
  "^(?<type>(?<size>[S|T]{0,1})(?<general_type>[A-Z]{1}[1-9]{1}))(?<add>[a-z]{0,2})$",
);
const MARTINI3_BEADS_RADIUS = { S: 2.3, T: 1.91, R: 2.64 };

const MARTINI2_BEADS_RADIUS = { R: 2.64, S: 2.41, T: 1.8 };
const BEADS_RADIUS_EXCEPTIONS: { [bead: string]: number } = { D: 1 };
export const MARTINI2_BEADS_REGEX = new RegExp(
  "^(?<size>[S|T]{0,1})(?<type>[A-Z]{1}(?<add>[a-z0-9]*))$",
);
const MARTINI2_REGEX_EXCEPTIONS = ["AC1", "AC2"];

const FF_TO_SCHEMES: {
  [ff: string]: {
    color: { [beadType: string]: { color: string; hydrophobicity: number } };
    radius: { [key in BeadSize]: number };
    regex: RegExp;
    beadsAA?: { [bead: string]: { aminoAcid: string; atomName: string }[] };
    regexExceptions?: string[];
  };
} = {
  martini3IDP: {
    color: martini3ColorScheme,
    radius: MARTINI3_BEADS_RADIUS,
    regex: MARTINI3_BEADS_REGEX,
    beadsAA: martini3BeadsAA,
  },
  "martini3 lipidome": {
    color: martini3ColorScheme,
    radius: MARTINI3_BEADS_RADIUS,
  ...
  martini22: {
    color: martini2ColorScheme,
    radius: MARTINI2_BEADS_RADIUS,
    regex: MARTINI2_BEADS_REGEX,
    regexExceptions: MARTINI2_REGEX_EXCEPTIONS,
  },
  martini22p: {
    color: martini2ColorScheme,
    radius: MARTINI2_BEADS_RADIUS,
...
export function getBeadGeneralTypeAndAA(
  ff: AvailableForceFields,
):
  | { [beadType: string]: { aminoAcid: string; atomName: string }[] }
  | undefined {
  const returnObj: {
    [beadType: string]: { aminoAcid: string; atomName: string }[];
  } = {}; //{bead : AA[]}
...
export default class MartiniSchemes {
  ...
  protected _getColor(
    ...
    if (bead.name === "other") {
      if (bead.moleculeName && bead.moleculeName in MOLECULE_EXCEPTIONS_COLOR)
        return MOLECULE_EXCEPTIONS_COLOR[bead.moleculeName];
      ...
      const beadColorName =
        regexExc && regexExc.includes(bead.colorToTake)
          ? bead.colorToTake
          : bead.colorToTake.match(beadRegex)?.groups?.type;
  ...
  protected _createColorMap(ff: string, beads: Bead[]): string[] {
    ...
    const colorScheme =
      ff in FF_TO_SCHEMES ? FF_TO_SCHEMES[ff].color : undefined;
    if (!colorScheme) colorMap.fill(DEFAULT_COLOR, 0, beads.length);
      ...
      for (const [idx, bead] of beads.entries()) {
        const color = this._getColor(bead, {
          color: colorScheme,
          regex: beadRegex,
  ...
  protected _createSchemeId(colorMap: string[]): string {
    ...
    const colorSelection = Object.keys(colorObj).map((color) => [
      color,
      "@" + colorObj[color].join(","),
    ]);
  ...
  protected _cloneAndModifyColorScheme(
    colorMap: string[],
    toModify: { [atomIdx: number]: string },
  ) {
  ...
  protected _createRadiusMap(ff: AvailableForceFields, beads: Bead[]) {
    return beads.map((bead) => {
      ...
      const beadSize = (
        regMatch?.groups?.size ? regMatch?.groups?.size : "R"
      ) as BeadSize;
      if (bead.name in BEADS_RADIUS_EXCEPTIONS)
        return BEADS_RADIUS_EXCEPTIONS[bead.name];
      return FF_TO_SCHEMES[ff].radius[beadSize];
    });
  }

  getProteinColorScheme(ff: AvailableForceFields, beads: Bead[]): string {
    if (!(ff in this._originalColorMap))
      this._originalColorMap[ff] = this._createColorMap(ff, beads);
    if (!(ff in this._originalColorSchemeId))
      this._originalColorSchemeId[ff] = this._createSchemeId(
        this._originalColorMap[ff],
      );
    return this._originalColorSchemeId[ff];
  }

  getProteinRadiusScheme(
    ff: AvailableForceFields,
    beads: Bead[],
    factor: number = 1,
  ) {
    if (!(ff in this._originalRadiusMap))
      this._originalRadiusMap[ff] = this._createRadiusMap(ff, beads);
  ...
  applyRadiusFactor(ff: AvailableForceFields, factor: number) {
    if (ff in this._originalRadiusMap)
      return this._originalRadiusMap[ff].map((radius) => radius * factor);
  }

  highlightAtomColorScheme(
    ff: AvailableForceFields,
    toHighlight: { [atomIdx: number]: string },
  ): string {
    if (ff in this._originalColorMap)
      return this._cloneAndModifyColorScheme(
        this._originalColorMap[ff],
        toHighlight,
      );
