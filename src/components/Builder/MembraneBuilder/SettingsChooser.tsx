...
class SettingsChooser extends React.Component<SCProps, SCState> {
  ...
  next = () => {
    ...
    this.props.onSettingsChoose({
      ...
      area_per_lipid_upper: this.props.hasUpperLayer
        ? Number(area_per_lipid_upper)
        : undefined,
  ...
  get box_is_error() {
    const items = this.state.box_size
      .split(",")
      .map((e) => (!e ? NaN : Number(e)));

    // test: Number is not NaN, not < 0, not a floating point number, and dimension is 3, 6 or 9.
    return (
      items.some(
        (e) => isNaN(e) || e < 0 || e.toString().indexOf(".") !== -1,
      ) || ![3, 6, 9].includes(items.length)
    );
  }
  ...
  get polarizable_compatibility(): boolean {
    ...
    if (
      water_polarization ===
      Settings.martinize_variables.force_fields_info[this.props.forceField]
        .polarizable
    )
      return true;
    else return false;
  ...
  renderErrorText(text: string) {
    switch (text) {
      case "insane_crash":
        return "INSANE run failed";
      case "gromacs_crash":
        return "GROMACS failed to compile membrane topology";
      case "top_file_crash":
        return "System couldn't be properly created";
      default:
  ...
  renderError() {
    ...
    if (!error || error === true) {
      return (
        <Typography color="error">Run failed with an unknown error.</Typography>
      );
    }
    ...
    if (error.error === "PARAMS_VALIDATION_ERROR") {
      return (
        <div>
          <Typography color="error">Error with parameters</Typography>
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        {error && (
          <Box textAlign="center" style={{ marginBottom: "1rem" }}>
            {this.renderError()}
          </Box>
        )}
          ...
          <TabPanel value="yes">
            ...
            <div>
              <TextField
                ...
                onChange={(evt) =>
                  this.setState({ box_size: evt.target.value })
                }
            ...
            {this.props.addLipids && (
              <React.Fragment>
                ...
                <Box
                  display="grid"
                  gridTemplateColumns={
                    this.props.hasUpperLayer ? "1fr 1fr" : "1fr"
                  }
                  gridGap="4%"
                >
                  {this.props.hasUpperLayer && (
                    <Box width="100%">
                      <TextField
                        value={this.state.area_per_lipid_upper}
                        onChange={(e) =>
                          this.setState({
                            area_per_lipid_upper: e.target.value,
                          })
                        }
                  ...
                  <Box width="100%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ area_per_lipid: e.target.value })
                      }
            ...
            {this.props.addMolecule === "true" && (
              <React.Fragment>
                ...
                <Box display="grid" gridTemplateColumns="1fr" gridGap="10px">
                  ...
                  {this.state.rotate_mode === "angle" && (
                    <Box width="100%">
                      <TextField
                        value={this.state.rotate_angle}
                        onChange={(e) =>
                          this.setState({ rotate_angle: e.target.value })
                        }
              ...
              <Box display="grid" gridTemplateColumns="1fr" gridGap="4%">
                <Box width="100%">
                  <TextField
                    ...
                    onChange={(e) =>
                      this.setState({ salt_concentration: e.target.value })
                    }
                ...
                <Box width="100%">
                  ...
                  <RadioGroup
                    row
                    name="Solvent type"
                    value={this.state.solvent_type}
                    onChange={(e) =>
                      this.setState({ solvent_type: e.target.value })
                    }
                  >
                    <FormControlLabel
                      value="W"
                ...
                >
                  <Box width="48%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ random_kick_size: e.target.value })
                      }
                  ...
                  <Box width="48%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ bead_distance: e.target.value })
                      }
                ...
                <Box display="grid" gridTemplateColumns="1fr 1fr" gridGap="4%">
                  <Box width="100%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({
                          grid_spacing_orientation: e.target.value,
                        })
                      }
                      label="Grid spacing"
                      helperText="Must be greater than 0"
                  ...
                  <Box width="100%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ hydrophobic_ratio: e.target.value })
                      }
                ...
                <Box display="grid" gridTemplateColumns="1fr 1fr" gridGap="4%">
                  <Box width="100%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ fudge_factor: e.target.value })
                      }
                  ...
                  <Box width="100%">
                    <TextField
                      ...
                      onChange={(e) =>
                        this.setState({ shift_protein: e.target.value })
                      }
                      label="Shift protein"
