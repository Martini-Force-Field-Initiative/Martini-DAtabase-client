...
export class BondsRepresentation {
    ...
    getAtomProxy(index:number):BondAtomProxy {
     // debugLog(`[BondRepresentation:getAtomProxy] index is (${typeof(index)}) ${index}`);
...
//      debugLog("[BondsRepsentation:render] Starting")
  
      const shape = new ngl.Shape("add-bonds");
      const upper_mode = mode.toLocaleUpperCase();
      
      //debugLog(`[BondRepresentation:render] is working with following bonds storage structure:`);
      //debugLog(bonds);

      for (const [chain_atom1, chain_atom2 ] of bonds){
        //debugLog(`[BondRepresentation:render] processing ${chain_atom1} ${chain_atom2}`);
        ...
        //debugLog(`[BondRepresentation:render] translated to ${chain1},${atom1} ${chain2},${atom2}`);
      // We ignore the chain, offset in atom count through chain ID coor arrays is handle by getCoordinate                 
        ...
        //debugLog(`[BondRepresentaion:render] getting proxies of atom1, atom2: ${atom1}, ${atom2}`);
        const ap1 = this.getAtomProxy((atom1 as number) - 1);
        const ap2 = this.getAtomProxy((atom2 as number) - 1);
        //debugLog(`[BondRepresentaion:render] ap1 as ${ap1.name} ${ap1.resname} ${ap1.resno}, ${ap1.chainname}`);
        //debugLog(`[BondRepresentaion:render] ap2 as ${ap2.name} ${ap2.resname} ${ap2.resno}, ${ap2.chainname}`);
       ...
       // debugLog("[BondRepresentation:render] lookup ", atom1, atom2, ap1.chainname, ap2.chainname)
        if (hightlight_predicate) {
          if(hightlight_predicate(atom1, atom2, ap1.chainname, ap2.chainname)) { 
            debugLog(`[BondsRepsentation:render] Found link to highlight`);
