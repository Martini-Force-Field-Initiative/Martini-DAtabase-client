...
import {
  getBeadsLegend,
  getBeadGeneralTypeAndAA,
} from "../../martiniNglSchemes";
...
export default class BeadsLegend extends React.Component<
  BeadsLegendProps,
  BeadsLegendState
> {
  ...
  drawLegend() {
    ...
    const charged_end_position =
      this.createChargeBeadsLegend(beads_end_position);
    const go_part_end_position =
      this.createMiscellaneousBeadsLegend(charged_end_position);
    this.svg
      .append("text")
      .text(`Martini force field : ${this.props.ff}`)
      .attr("y", go_part_end_position + 40)
  ...
  createHydrophobicityColorScale() {
    ...
    const linearScale = d3.scaleLinear().domain([-30, 20]).range([0, 500]);
    ...
    this.svg.append("text").text("Hydrophobicity scale").attr("y", 10);
    ...
    if (legendValues) {
      const beadsToPlot = Object.keys(legendValues).filter(
        (beadName) =>
          beadName.startsWith("C") ||
          beadName.startsWith("N") ||
          beadName.startsWith("P"),
      );
      ...
      this.svg
        ...
        .attr(
          "cy",
          (d: string) => 18 + linearScale(legendValues[d].hydrophobicity),
        )
        .attr("cx", 30)
  ...
  createBeadsThreeColumns() {
    ...
    if (this.legendValues) {
      const smallBeads = Object.keys(this.legendValues).filter(
        (beadName) =>
          beadName.startsWith("SC") ||
          beadName.startsWith("SN") ||
          beadName.startsWith("SP"),
      );
      this.svg
        .selectAll("small-dot")
        .data(smallBeads)
      ...
      const tinyBeads = Object.keys(this.legendValues).filter(
        (beadName) =>
          beadName.startsWith("TC") ||
          beadName.startsWith("TN") ||
          beadName.startsWith("TP"),
      );
      this.svg
        .selectAll("tiny-dot")
        .data(tinyBeads)
  ...
  createMiscellaneousBeadsLegend(start_position: number): number {
    const width = 25;
    const height = 20;
    const y_start = start_position + 35;
    const x_start = 0;
  ...
  createRect(color: string, position: number, legend: string) {
    const y =
      position === 0 ? 0 : position * this.rect_height + position * this.y_gap;
    this.svg
      .append("circle")
  ...
  triggerOpenLegend = (event: React.MouseEvent<HTMLButtonElement>) => {
    if (this.state.open) {
      ...
      this.setState({
        open: true,
        buttonText: "Close color legend",
        anchorElmt: event.currentTarget,
      });
    }
  };
  ...
  render() {
    return (
      <React.Fragment>
        <Button style={ButtonStyle} onClick={this.triggerOpenLegend}>
          {" "}
