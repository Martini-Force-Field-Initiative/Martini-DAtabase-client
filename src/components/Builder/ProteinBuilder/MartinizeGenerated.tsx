...
interface MartinizeGeneratedProps {
  ...
  goSiteVisible: boolean;
  onGoSiteVisibilityChange(_: any, checked: boolean): any;
...
export default class MartinizeGenerated extends React.Component<
  MartinizeGeneratedProps,
  MartinizeGeneratedStates
> {
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Dialog open={this.state.openWarning} fullWidth={true} maxWidth="lg">
          <DialogTitle>Gromacs encountered warnings : </DialogTitle>
          <DialogContent>
            <DialogContentText>{this.state.warnings}</DialogContentText>
          </DialogContent>
          <DialogActions>
            <Button
        ...
        <Dialog
          open={this.state.wantReset}
          onClose={() => this.setState({ wantReset: false })}
        >
          <DialogTitle>Restart molecule builder ?</DialogTitle>

          <DialogActions>
            <Button
              color="primary"
              onClick={() => this.setState({ wantReset: false })}
            >
              Cancel
            </Button>
        ...
        >
          <FaIcon redo-alt />{" "}
          <span style={{ marginLeft: ".6rem" }}>Restart builder</span>
        </Button>
        ...
        {this.props.allAtomName !== "NOTHING" && (
          <>
            {/* All Atom Settings */}
            <Typography variant="h6">All atom</Typography>
            ...
            <FormControl component="fieldset">
              <FormGroup>
                <FormControlLabel
                  control={
                    <Switch
                      checked={this.props.allAtomVisible}
                      onChange={this.props.onAllAtomVisibilityChange}
                      value="visible"
                    />
                  }
                  label="Visible"
        ...
        <Typography variant="h6">Coarse grained</Typography>
        ...
        {this.props.representations.includes("ball+stick") && (
          <React.Fragment>
            <Typography gutterBottom>Beads radius factor</Typography>
            <Slider
        ...
        <FormControl component="fieldset">
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
                  checked={this.props.coarseGrainedVisible}
                  onChange={this.props.onCoarseGrainedVisibilityChange}
                  value="visible"
                />
              }
              label="Visible"
        ...
        {this.props.virtualLinks && (
          <React.Fragment>
            <Typography variant="h6">
              Virtual {this.props.virtualLinks === "go" ? "Go" : "Elastic"}{" "}
              bonds
            </Typography>

            <Marger size=".5rem" />
            {this.props.virtualLinks === "go" && (
              <FormControl component="fieldset">
                <FormGroup>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={this.props.goSiteVisible}
                        onChange={this.props.onGoSiteVisibilityChange}
                        value="visible"
                      />
                    }
                    label="Show virtual sites"
                  />
                </FormGroup>
              </FormControl>
            )}
            {this.props.virtualLinks === "go" && (
              <Box alignContent="center" justifyContent="center" width="100%">
                ...
                >
                  <FaIcon pen />{" "}
                  <span style={{ marginLeft: ".6rem" }}>Edit</span>
                </Button>
              </Box>
            )}

            {this.props.virtualLinks === "elastic" && (
              <Box alignContent="center" justifyContent="center" width="100%">
                <Button
                  style={{ width: "100%" }}
                  color="primary"
            ...
            <FormControl component="fieldset">
              <FormGroup>
                <FormControlLabel
                  control={
                    <Switch
                      checked={this.props.virtualLinksVisible}
                      onChange={this.props.onVirtualLinksVisibilityChange}
                      value="visible"
                    />
                  }
                  label="Visible"
