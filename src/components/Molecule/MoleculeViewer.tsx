...
class MoleculeViewer extends React.Component<MVProps, MVState> {
  ...
  componentDidMount() {
    ...
    this.ngl = new NglWrapper(this.viewport_id, {
  ...
  protected async initStage() {
    ...
    const request: Promise<{
      ...
      top: string;
      itps: string[];
    }> = ApiHelper.request(this.getUrlFromId(), {
      mode: "json",
    });
    ...
    request
      .then(async ({ radius, pdb, top, itps }) => {
        ...
        return this.ngl
          .load(new Blob([pdb]), {
            ext: "pdb",
            name: this.props.id + ".pdb",
            coarse_grained: true,
          })
          .then((component) => {
            component.add<BallAndStickRepresentation>(
              "ball+stick",
    ...
    return (
      <div
        ...
        style={{
          ...
          minHeight: "300px",
          borderRadius: "8px",
          border: "1px #82828278 dashed",
          position: "relative",
        }}
      >
        <FormControlLabel
          style={{ marginLeft: "1em", marginTop: "1em" }}
          componentsProps={{ typography: { sx: { fontSize: 13 } } }}
          control={
            <Switch
              size="small"
              checked={this.state.autospin}
              onChange={this.changeAutospin}
            />
          }
          label="autospin"
        />
