      ...
      <Fragment>
        ...
        <div>
          ...
          {molecule.citation && <Typography className={classes.version}>
            When using this molecule, please cite:
            <IconButton aria-label="copy" size="small" onClick={() => navigator.clipboard.writeText(molecule.citation)}>
                <ContentCopy />
            </IconButton>
            <Alert variant="outlined" severity="info" icon={false}> {molecule.citation} </Alert>
          </Typography>}
        </div>
      </Fragment>
