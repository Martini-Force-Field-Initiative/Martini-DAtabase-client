...
export function setSVG(svgref: SVGElement) {
  Mysvg = svgref;
  GraphicalNodeGenerator.setDefinitionsSVG(Mysvg);
}
...
export function initSimulationOriginal(
  sizeNodenodeSize: number,
...
export function removeNodes(
  ...
  for (let nodeToRemove of nodesToRemove) {
    console.log(">>Current node to remove is:");
    ...
    d3.select(Mysvg)
      ...
      .filter((d: SimulationNode) => {
        ...
        console.log(`${d.id} === ${nodeToRemove.id}`);
...
export function addNodeToSVG_as_Gs(
  ...
  console.log("addNodeToSVG_as_Gs, new nodes input are:");
  console.log(newnodes);
  if (newnodes.length == 0) return;
  //console.log(GraphicalNodeGenerator);
  ...
  //console.log("Checking size b4 join of 'g.nodes'")
  //console.log(d3.select(Mysvg).selectAll('g.nodes').size());
  /*d3.select(Mysvg).selectAll('g.nodes').each(
        (d)=>console.log(d)
    )*/
