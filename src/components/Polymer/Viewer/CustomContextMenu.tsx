...
export default class CustomContextMenu extends React.Component<props> {
    ...
    if (node.links !== undefined) {
      for (let linkednode of node.links) {
        ...
        linkednode.links = linkednode.links!.filter(
          (nodeToRM: SimulationNode) => nodeToRM.id !== node.id,
        );
      }
    }
  ...
  removeSelectedNodes = (
    ...
    removeNodes(li, this.props.handleUpdate, () => {
        ...
        for (let linkednode of node.links) {
          if (listnodes.includes(linkednode)) {
            ...
            node.links = node.links!.filter(
              (nodeToRM: SimulationNode) => nodeToRM.id !== linkednode.id,
            );
            this.props.svg
              .selectAll("line")
              .filter(
  ...
  clear = () => {
    ...
    this.props.svg.selectAll("g.nodes").remove();
    this.props.handleUpdate();
  ...
  colapse = (group: SimulationGroup): void => {
    ...
    this.props.svg
      ...
      .filter(
        (link: any) =>
          listid.includes(link.source.id) || listid.includes(link.target.id),
      )
      .remove();
    ...
    this.props.svg
      ...
      .attr(
        ...
        d3
          ...
          .size(1000 * Math.sqrt(group.nodesD3!.data().length)),
      )
      .attr("fill", group.color!)
      .style("opacity", 0.7)
      .attr("id", function (d: SimulationGroup) {
        return d.id;
      })
      .attr("group", function (d: SimulationGroup) {
        return d.id;
      })
    ...
    return (
      ...
      >
        {selectedNodes.size() > 0 && (
          <div key={1}>
            <Typography>{selectedNodes.size()} nodes selected</Typography>
            <Divider />
            <MenuItem
              onClick={() => {
                this.removeLinksSelected(selectedNodes);
              }}
            >
            ...
            <MenuItem
              onClick={() => {
                this.removeSelectedNodes(selectedNodes);
              }}
            >
              {" "}
              Remove {selectedNodes.size()} selected nodes
            </MenuItem>
            <MenuItem
              onClick={() => {
                this.props.handlePaste(selectedNodes);
              }}
            >
              {" "}
              Paste {selectedNodes.size()} selected nodes
            </MenuItem>
            <MenuItem
              onClick={() => {
                selectedNodes.attr("class", "nodes");
              }}
            >
              Unselected
            </MenuItem>
        ...
        {this.props.lineClick && (
          <div>
            ...
            <MenuItem
              onClick={() => {
                this.removeThisLink(this.props.lineClick!);
              }}
            >
              {" "}
              Remove this link{" "}
        ...
        {this.props.nodeClick && (
          <div>
            <MenuItem
              onClick={() => {
                this.removeNodeLinks(this.props.nodeClick!);
              }}
            >
              Remove link
            </MenuItem>
            ...
            <MenuItem
              onClick={() => {
                this.giveConnexeNode(this.props.nodeClick!).attr(
                  "class",
                  "onfocus",
                );
              }}
        ...
        {this.props.svg
          ...
          <MenuItem
            onClick={() => {
              this.removeBadLinks(this.props.svg);
            }}
          >
            Remove bad links
          </MenuItem>
        ...
        {this.props.simulation.nodes.length === 0 && (
          <MenuItem
            onClick={() => {
              DownloadJson(this.props.simulation, this.props.forcefield);
            }}
          >
            Download Json
          </MenuItem>
